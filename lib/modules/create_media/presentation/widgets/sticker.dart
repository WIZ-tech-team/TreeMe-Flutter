...
class WaveSliderState extends State<WaveSlider> {
  ...
  void initState() {
    ...
    print(shortSize);
    widthSlider =
        (widget.widthWaveSlider < 50) ? (shortSize - 2 - 40) : widget.widthWaveSlider;
  ...
  int _getEndTime() {
    return ((_getBarEndPosition() + selectBarWidth) / (widthSlider / widget.duration))
        .ceilToDouble()
        .toInt();
  }

  String _timeFormatter(int second) {
    Duration duration = Duration(milliseconds: second);
